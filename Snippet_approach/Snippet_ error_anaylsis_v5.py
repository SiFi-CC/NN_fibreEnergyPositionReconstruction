...
def iterate_fwhm(arr_true, arr_reco, bins, filename, attribute):
    ...
    maxval      = np.max(arr_true)
    if attribute == "E":
        div         = 10/bins
        maxval      = 10
    div_arr     = np.arange(0, maxval, div)
...
def give_E_y(Y_test, f_X_test):
    y_true = np.zeros(Y_test.size//2)
    y_reco = np.zeros(Y_test.size//2)
    E_true = np.zeros(Y_test.size//2)
    E_reco = np.zeros(Y_test.size//2)    
    for i in range(Y_test.size//2):
                y_true[i] = Y_test[i][1]    * 100  
                y_reco[i] = f_X_test[i][1]  * 100
                E_true[i] = Y_test[i][0]    * 2.6486268267035484
                E_reco[i] = f_X_test[i][0]  * 2.6486268267035484
...
def true_false_stats(true, reco):
    ...
    # returns values and percentages
